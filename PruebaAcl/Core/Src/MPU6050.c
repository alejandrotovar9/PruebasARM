...
void MPU6050_Init (void)
{
  ...
  {
	   unsigned char mensaje[30] = "MPU6050 No encontrado!!!";
	   HAL_UART_Transmit(&huart2, mensaje, strlen(mensaje), 0xFFFF);
	   while(1){}
  }
}
...
void MPU6050_Read_Accel (void)
{
  ...
  uint8_t buftemp[64] = { 0 };

  // Read 6 BYTES of data starting from ACCEL_XOUT_H register
  HAL_I2C_Mem_Read(&hi2c2, MPU6050_ADDR, ACCEL_XOUT_H_REG, 1, Rec_Data, 6, 1000);

  //sprintf(buftemp, "%d, Valores obtenidos \r\n", Rec_Data);
  //sHAL_UART_Transmit(&huart2, buftemp, sizeof(buftemp), 10);
